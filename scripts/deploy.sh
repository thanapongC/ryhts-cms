...
CMS_IMAGE="${IMAGE_PREFIX}:ryhtscms"
WEB_IMAGE="${IMAGE_PREFIX}:ryhtsweb"
...
CMS_DOMAIN="${CMS_DOMAIN:-ryhtscms.icute.site}"
WEB_DOMAIN="${WEB_DOMAIN:-ryhtsweb.icute.site}"
...
if [[ "$SSL_ONLY" == "false" ]]; then
  ...
  dry_run docker build --platform linux/amd64 --provenance=false --sbom=false \
    --build-arg "PUBLIC_STRAPI_URL=${PUBLIC_STRAPI_URL:-http://ryhts-cms:1337}" \
